...
function buildEd25519SecretBody(pubBody, seedBytes) {
    // Use pgpMPI so the bit count accurately reflects the actual seed value.
    const secretMPI = pgpMPI(seedBytes);
...
async function buildSelfSigPacket(pubBody, uidBytes, pkAlgo, timestamp, doSign) {
    ...
    // SHA-256 digest — used for left-two-bytes field AND passed to doSign
    const digest  = new Uint8Array(await crypto.subtle.digest('SHA-256', hashInput));
    const leftTwo = digest.slice(0, 2);

    // doSign receives both raw hashInput (for RSA which hashes internally) and
    // the pre-computed digest (for EdDSA which receives the hash as the message)
    const sigMPIs = await doSign(hashInput, digest);
...
async function generateEd25519GPGPair(uid) {
    ...
    const sigPacket = await buildSelfSigPacket(pubBody, uidBytes, 22, timestamp, async (_hashInput, digest) => {
        // OpenPGP EdDSA: sign the SHA-256 digest of the hash input, not the raw data.
        // Web Crypto Ed25519 signs the message as-is (PureEdDSA internally SHA-512s it),
        // so we pass the 32-byte SHA-256 digest as the message.
        const sigRaw = new Uint8Array(await crypto.subtle.sign({ name: 'Ed25519' }, keyPair.privateKey, digest));
...
async function generateRSAGPGPair(bits, uid) {
    ...
    const sigPacket = await buildSelfSigPacket(pubBody, uidBytes, 1, timestamp, async (hashInput, _digest) => {
        // RSASSA-PKCS1-v1_5 with SHA-256: Web Crypto hashes internally, pass full hashInput.
